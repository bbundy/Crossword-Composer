...
class Puzzle:
    ...
    @classmethod
    def fromDicts(cls, author, title, across_answers, down_answers, across_pos, down_pos, across_clues, down_clues):
        this = cls()
        this.type = 'cryptic'
        this.author = author
        this.title = title
        ...
        for i in range(15):
            ...
            for j in range(15):
                row.append('.')
            mat.append(row)
        for k in across_answers.keys():
            i = across_pos[k][0]
            j = across_pos[k][1]
            for m in range(len(across_answers[k])):
                mat[j][i+m] = across_answers[k][m:m+1]

        for k in down_answers.keys():
            i = down_pos[k][0]
            j = down_pos[k][1]
            for m in range(len(down_answers[k])):
                mat[j+m][i] = down_answers[k][m:m+1]

        for row in mat:
            this.row.append(''.join(row))
        this = Puzzle.getFromRows(this)
        this.setGrid()

        for clue in this.across:
            clue.clue = across_clues[int(clue.num)].encode("latin-1")

        for clue in this.down:
            clue.clue = down_clues[int(clue.num)].encode("latin-1")
